...
void NuPlayer::RTSPSource::connect() {
    ...
    sp<AMessage> notifyStart = mNotify->dup();
...
void NuPlayer::RTSPSource::pause() {
    ...
    for (size_t index = 0; index < mTracks.size(); index++) {
        ...
        if (source != NULL && source->finished(mediaDurationUs)) {
            return;
        }
    }
    mHandler->pause();
...
bool NuPlayer::RTSPSource::haveSufficientDataOnAllTracks() {
    ...
    int64_t mediaDurationUs = 0;
    getDuration(&mediaDurationUs);
    if ((mAudioTrack != NULL && mAudioTrack->finished(mediaDurationUs))
            || (mVideoTrack != NULL && mVideoTrack->finished(mediaDurationUs))) {
        return true;
    }
...
status_t NuPlayer::RTSPSource::dequeueAccessUnit(
    ...
    if (!source->hasBufferAvailable(&finalResult)) {
        if (finalResult == OK) {
            ...
            if (source->finished(mediaDurationUs)) {
            ...
            if (!(otherSource != NULL && otherSource->finished(mediaDurationUs))) {
