...
status_t SoftVPXEncoder::initEncoder() {
    ...
    codec_return = vpx_codec_control(mCodecContext, VP8E_SET_TOKEN_PARTITIONS, mDCTPartitions);
    if (codec_return != VPX_CODEC_OK) {
        ALOGE("Error setting codec parameter");
        return UNKNOWN_ERROR;
    }
    ...
    switch (indexFull) {
        ...
        case OMX_IndexParamVideoVp8: {
            OMX_VIDEO_PARAM_VP8TYPE *vp8Params =
                ...
                if (vp8Params->nPortIndex != 1) {
                    return OMX_ErrorUnsupportedIndex;
                }

                // vp8 encoder has no profiles or levels
                vp8Params->eProfile = OMX_VIDEO_VP8ProfileMain;
                vp8Params->eLevel = OMX_VIDEO_VP8Level_Version0;
