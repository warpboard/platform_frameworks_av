...
extern const Word16 vogrid[GRID_POINTS+1];
/* local function */
    ...
    )
{
    ...
    *-------------------------------------------------------------*/
    ...
    __asm__ volatile (
        ...
        "sub        %[tmp1],    %[t0],      %[t1]       \n\t"
        "sub        %[tmp3],    %[t2],      %[t3]       \n\t"
        "shra_r.w   %[tmp0],    %[tmp0],    1           \n\t"
        "shra_r.w   %[f2t0],    %[tmp1],    1           \n\t"
        "shra_r.w   %[tmp2],    %[tmp2],    1           \n\t"
        "shra_r.w   %[f2t1],    %[tmp3],    1           \n\t"
        "sh         %[tmp0],    0(%[f1])                \n\t"
        "sh         %[f2t0],    0(%[f2])                \n\t"
        "sh         %[tmp2],    2(%[f1])                \n\t"
        "sh         %[f2t1],    2(%[f2])                \n\t"
        ...
        : [tmp0] "=&r" (tmp0), [tmp1] "=&r" (tmp1), [tmp2] "=&r" (tmp2),
          [tmp3] "=&r" (tmp3), [f2t0] "=&r" (f2t0), [f2t1] "=&r" (f2t1),
          [t0] "=&r" (t0), [t1] "=&r" (t1), [t2] "=&r" (t2),
          [t3] "=&r" (t3)
        : [a] "r" (a), [f1] "r" (f1), [f2] "r" (f2)
        : "memory"
    );
    f1[NC] = a[NC];
#else
