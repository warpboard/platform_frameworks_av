...
SDPLoader::SDPLoader(const sp<AMessage> &notify, uint32_t flags, bool uidValid, uid_t uid)
    ...
    mNetLooper->start(false /* runOnCallingThread */,
                      false /* canCallJava */,
                      PRIORITY_HIGHEST);
    mReflector = new AHandlerReflector<SDPLoader>(this);
    mNetLooper->registerHandler(mReflector);
}

void SDPLoader::load(const char *url, const KeyedVector<String8, String8> *headers) {
    sp<AMessage> msg = new AMessage(kWhatLoad, mReflector->id());
...
void SDPLoader::onLoad(const sp<AMessage> &msg) {
    ...
    uint8_t* data = NULL;
    if (err == OK && !mCancelled) {
        data = (uint8_t*) malloc(sdpSize);
        if (data == NULL) {
            ALOGE("SDP alloc failed");
            err = UNKNOWN_ERROR;
        }
    }

    if (err == OK && !mCancelled) {
        ssize_t readSize = mHTTPDataSource->readAt(0, data, sdpSize);
        ...
        if (readSize < 0) {
            ...
            if (desc == NULL || !desc->setTo(data, (size_t)readSize)) {
                err = UNKNOWN_ERROR;
