...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitAC3() {
    ...
    static uint32_t sample_freq [4] = {48000,44100,32000,0};
    static uint32_t channels[8] ={2,1,2,3,3,4,4,5};
    ...
    while(offset + 7 <= mBuffer->size()) {
        ...
        if(mFormat == NULL){
            mFormat = static_cast <MetaData *> (new MetaData);
